...
local function table_index(t, val)
    for i, v in ipairs(t) do
        if v == val then return i - 1 end  -- 0-based index to match Python
    end
    return nil
end
...
function Value:__add(other)
    if type(other) == "number" then other = Value.new(other) end
...
local function softmax(logits)
    ...
    for i = 2, #logits do
        if logits[i].data > max_val then max_val = logits[i].data end
    end
    local exps = {}
    ...
    local total = exps[1]
    for i = 2, #exps do total = total + exps[i] end
    local out = {}
    for _, e in ipairs(exps) do out[#out + 1] = e / total end
    return out
end

local function rmsnorm(x)
    local ms = Value.new(0)
    for _, xi in ipairs(x) do ms = ms + xi * xi end
    ms = ms * (1 / #x)
    local scale = (ms + 1e-5) ^ -0.5
    local out = {}
    for _, xi in ipairs(x) do out[#out + 1] = xi * scale end
    return out
end
...
local function gpt(token_id, pos_id, keys, values)
    ...
    for li = 0, n_layer - 1 do
        ...
        for h = 0, n_head - 1 do
            local hs    = h * head_dim
            local q_h   = {}
            for j = 1, head_dim do q_h[j] = q[hs + j] end
            local k_h   = {}
            for t = 1, #layer_keys do
                k_h[t] = {}
                for j = 1, head_dim do k_h[t][j] = layer_keys[t][hs + j] end
            end
            local v_h   = {}
            for t = 1, #layer_values do
                v_h[t] = {}
                for j = 1, head_dim do v_h[t][j] = layer_values[t][hs + j] end
            end
            -- Attention logits
            local attn_logits = {}
            for t = 1, #k_h do
...
for step = 1, num_steps do
    ...
    for i = 2, #losses do loss_sum = loss_sum + losses[i] end
...
for sample_idx = 1, 20 do
    ...
    for pos_id = 0, block_size - 1 do
        ...
        local weights = {}
        for _, p in ipairs(probs) do weights[#weights + 1] = p.data end
        token_id = weighted_choice(weights) - 1  -- back to 0-based
        if token_id == BOS then break end
